  ...
  <el-input v-model="searchInfo.nickname" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column align="left" label="品种" prop="breed" width="120" />

            <el-table-column align="left" label="是否绝育" prop="is_neutered" width="120">
    <template #default="scope">{{ formatBoolean(scope.row.is_neutered) }}</template>
</el-table-column>
            <el-table-column align="left" label="体重" prop="weight" width="120" />

            <el-table-column align="left" label="过敏/禁忌" prop="allergy" width="120" />

            <el-table-column label="照片" prop="avatar" width="200">
    <template #default="scope">
      <el-image preview-teleported style="width: 100px; height: 100px" :src="getUrl(scope.row.avatar)" fit="cover"/>
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updatePetAnimalsFunc(scope.row)">编辑</el-button>
    ...
    {{ detailForm.is_neutered }}
</el-descriptions-item>
                    <el-descriptions-item label="体重">
    {{ detailForm.weight }}
</el-descriptions-item>
                    <el-descriptions-item label="过敏/禁忌">
    {{ detailForm.allergy }}
</el-descriptions-item>
...
const onSubmit = () => {
  elSearchFormRef.value?.validate(async(valid) => {
    if (!valid) return
    page.value = 1
    if (searchInfo.value.is_neutered === ""){
        searchInfo.value.is_neutered=null
    }
    getTableData()
  })
}
...
const getTableData = async() => {
  const table = await getPetAnimalsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deletePetAnimalsByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updatePetAnimalsFunc = async(row) => {
    const res = await findPetAnimals({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deletePetAnimalsFunc = async (row) => {
    const res = await deletePetAnimals({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
