...
<template>
  ...
  <el-input class="!w-40" v-model.number="searchInfo.startPhone" placeholder="最小值" />
  —
  <el-input class="!w-40" v-model.number="searchInfo.endPhone" placeholder="最大值" />
</el-form-item>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="日期" prop="CreatedAt" width="180">
            ...
            <el-table-column align="left" label="渠道" prop="channel" width="120" />

            <el-table-column align="left" label="标签" prop="tags" width="120" />

            <el-table-column align="left" label="账户余额" prop="balance" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" :min-width="appStore.operateMinWith">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updatePetCustomersFunc(scope.row)">编辑</el-button>
...
const getTableData = async() => {
  const table = await getPetCustomersList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deletePetCustomersByIds({ IDs })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updatePetCustomersFunc = async(row) => {
    const res = await findPetCustomers({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deletePetCustomersFunc = async (row) => {
    const res = await deletePetCustomers({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
